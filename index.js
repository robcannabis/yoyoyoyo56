...
client.on('ready', async () => {
 ...
 const num = parseFloat((Math.random() * 0.2 + 0.1 + Number.EPSILON).toFixed(1));
  ...
  setInterval(() => {
    ...
    const ddds = ['https://www.instagram.com/roo_kie19?igsh=MThud2pzcGs3Zmw1bQ==']; // ลิ้ง button1

   const iooi = ['SUMMER']; // ชื่อ button 2
   const iiio = ['https://discord.gg/ubHBX9pzQk']; // ลิ้ง button 2
...
//change[Math.floor(Math.random()*change.length)]; // รูปใหญ่
    ...
    const r = new Discord.RichPresence()
      ...
      .setName(`RobShop`)
      .setDetails(`ความรักก็เหมือน อิเล็กตรอนกับนิวเคลียร์ทำได้แค่อยู่รอบๆเข้าใกล้ไม่ได้`)
